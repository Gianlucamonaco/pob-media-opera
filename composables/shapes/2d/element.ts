...
import { breakText, fadeText, wrapText } from "~/composables/utils/string";
  ...
  draw() {
    ...
    this.data.forEach((item: any, i) => {
      ...
      if (shape === Shape2DType.RECTANGLE) {
        ...
        else if (style.textBreak) {
          const lineHeight = style.lineHeight || fontSize;
          breakText(this.ctx, content, 0, 0, style.textBreak, lineHeight);
        }

        else if (style.revealFade) {
          fadeText(this.ctx, content, 0, 0, item.params);
        }
