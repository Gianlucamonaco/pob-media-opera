...
export const sceneScripts: Partial<Record<Scenes, Scene3DScript>> = {
  [Scenes.ASFAY]: {
    ...
    update: (engine) => {
      ...
      const { pad1, pad2, pad3 } = midiState.pads;
      ...
      _input = {
        rectVisibilityChance: knob1 || drums.loudness || harmonies.loudness,
        ...
        visibilityTrigger: drums.onOff || harmonies.onOff || pad3,
      }
      ...
      const textVisibilityChance = 0.05 * _input.textVisibilityChance * _input.textVisibilityFactor;
      ...
      const cameraZoom = ((_state.targetDistance || distance) - distance) * 0.001;
      ...
      if (visibilityTrigger) {
        ...
        elements.grid.data.forEach((_, i) => {
          if (chance(visibilityChance) && !ended) {
  ...
  [Scenes.ASSIOMA]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        tunnelNarrowFactor: knob3 || snare.loudness,
      ...
      const tunnelSpeedVariation = mapClamp(_input.tunnelSpeedVariation, 0.5, 0.7, -0.25, 1) * _input.tunnelSpeedFactor * 3;
      const connectionRandomness = _input.connectionRandomness * 0.2;
      const pulseFrequencyPrimary = _input.pulseFrequency1 * 2.3;
      const pulseFrequencySecondary = _input.pulseFrequency2 * 1.6;
  ...
  [Scenes.DATASET]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        scanCountFactor: keysClem.pitch || knob3,
      }
      ...
      const cameraSpeedFactor = 1 + _input.cameraSpeedFactor * 3;
  ...
  [Scenes.ESGIBTBROT]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        speedFactor1: keysClem.loudness || knob1,
        speedFactor2: keys.loudness || knob2,
      ...
      const structureSpeed = 0.5 + (_input.speedFactor1 + _input.speedFactor2) * 0.5;
      ...
      const rectPrimaryDeformationSpeed = _input.pulseFrequency1 * 1;
      const rectSecondaryDeformationSpeed = _input.pulseFrequency2 * 2.5;
      const rectPrimaryDeformationInterval = 0.03085;
      const rectSecondaryDeformationInterval = 0.22;

      _state.pulseFrequency1 = lerp(_state.pulseFrequency1, rectPrimaryDeformationSpeed, 0.003);
      _state.pulseFrequency2 = lerp(_state.pulseFrequency2, rectSecondaryDeformationSpeed, 0.003);
      ...
      elements.structure.data.forEach((rect, i) => {
        ...
        if (rect.motionSpeed) {
          rect.position.z += rect.motionSpeed.position.z * structureSpeed;
        }
        
        const mixedFrequencies = Math.sin(BASE_FREQ * _state.pulseFrequency1 * 0.1 + i * rectPrimaryDeformationInterval)
                               * Math.sin(BASE_FREQ * _state.pulseFrequency2 * 0.1 + i * rectSecondaryDeformationInterval)
  ...
  [Scenes.FUNCTIII]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        scanChance: knob6 || drums.loudness,
        scanMinX: knob1 || drums.loudness,
        scanMaxX: knob1 || snare.loudness,
        scanMinY: knob2 || harmonies.loudness,
        scanMaxY: knob2 || bassDrum.loudness,
        rectRotationX: texture.loudness,
        narrowFactor: bass.loudness || knob3,
        slopeFactor: keys.pitch || keys.centroid || knob4,
        distortionFactor: knob5,
      }

      // Constants
      const BASE_FREQ = time * 0.001
      const VISIBILITY_RANGE_X = { min: -2000, max: 2000 };
      const VISIBILITY_RANGE_Z = { min: -2000, max: 500 };
      const SLOPE_FACTOR = { top: 50, bottom: -150 }; // slope Y of the top and bottom layer
      const GRID_OFFSET_Z = 2000;

      if (!ended) {
        _state.distortion = lerp(_state.distortion, _input.distortionFactor, 0.005)
      }
  ...
  [Scenes.GHOSTSSS]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        gridDistortion1: knob3 || harmonies.loudness,
        gridDistortion2: knob4 || keys.loudness,
        gridDistortionCenter: knob1 || snare.pitch,
        gridDistortionDepth: knob2 || bass.pitch,
        triggerCountFactor: knob5,
      ...
      // Computed audio values + MIDI
      const primaryDistortionIntensity = _input.gridDistortion1 * DISTORTION_AMPLITUDE * 1;
      ...
      // Reset 
      if (ended && _state.targetProgress == 1) {
  ...
  [Scenes.MITTERGRIES]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        globalSpeed: keysClem.loudness,
        speedFactor: knob1,
        rowFactor: keysClem.pitch,
        rowFactorIntensity: knob2,
        rowFactor1: brass.loudness,
        rowFactor2: woodwinds.loudness,
      ...
      // Computed audio values + MIDI
      const speedFactor = Easing.EXPO_IN(0.2 + _input.globalSpeed * (1 + _input.speedFactor)) * MAX_GLOBAL_SPEED;
      ...
      const rowIntensityFactors = rowSpeedFactors.map((row, i) => row + Math.abs(i - _input.rowFactor * rowSpeedFactors.length) * _input.rowFactorIntensity);
  ...
  [Scenes.MTGO]: {
    ...
    update: (engine, time) => {
      ...
      const maxPoints = Math.min(Math.floor(currentBar()/2) + 1, elements.main.data.length);

      // --- 2. GLOBAL & CAMERA SECTION ---
      const { azimuth, polar } = engine.getCameraAngles();
      const cameraAngleX = azimuth + cameraSpeedFactorX + Easing.SINE_IN(_input.cameraRotationFactor) * 0.25;
  ...
  [Scenes.RFBONGOS]: {
    ...
    update: (engine, time) => {
      ...
      const cameraRotationFactor = currentBar() * 2 * _input.cameraRotationFactor;
      ...
      for (let i = 0; i < visibilityTriggers.length; i++) {
        ...
        if (visibilityTriggers[i]) {
          ...
          _state.visibilityInterval[i] = mapQuantize(visibilityIntervals[i], 0.85, 0.60, INTERVAL_RANGE.min, INTERVAL_RANGE.max, true);
          _state.visibilityProgress[i] = 1;

          _state.visibilityRow[i] = mapQuantize(visibilityRows[i], 0.4, 0.8, 1, rows);
        }
      }
  ...
  [Scenes.SISTEMA]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        globalSpeedFactor: knob1 || texture.pitch,
        globalPositionFactor: harmonies.centroid,
        speedFactor1: drums.loudness || knob2,
        positionFactor1: drums.pitch,
        speedFactor2: overhead.loudness || knob3,
        positionFactor2: overhead.pitch,
        speedFactor3: texture.loudness || knob4,
        positionFactor3: texture.pitch,
        speedFactor4: brass.loudness || knob5,
        positionFactor4: brass.pitch,
        speedFactor5: woodwinds.loudness || knob6,
        positionFactor5: woodwinds.pitch,
      }
  ...
  [Scenes.SOLO_02]: {
   init: (engine) => {
    ...
    update: (engine, time) => {
      ...
      _input = {
        speedFactor1: brass.pitch || knob1,
        speedFactor2: knob3 || brass.centroid,
        scaleFactor1: brass.loudness || knob2,
        scaleFactor2: knob4 || brass.flatness,
        frequencyFactor: brass.loudness || knob5,
      }
      ...
      const cols = elements.grid?.config.layout.dimensions?.x || 1;
      const scaleFactors = [ _input.scaleFactor1, _input.scaleFactor2 ];
      const waveFrequency = BASE_FREQ * 0.01 + _state.frequencyProgress;
      const motionFrequency = BASE_FREQ * 0.01 + _state.frequencyProgress;
      const frequencyFactor = _input.frequencyFactor * rows;
      ...
      elements.grid?.data.forEach((rect, i) => {
        ...
        const scaleFactor = (0.35 - Math.abs(row - rows * scaleFactors[0]) * 0.12) + (0.35 - Math.abs(row - rows * scaleFactors[1]) * 0.09);
        const midiFactorY = Math.sin(scaleFactors[0] + row * 0.5) * Math.floor(Math.sin(Math.PI * 0.1 * row * frequencyFactor) * 3) + scaleFactors[0];
        const midiFactorX = 0.01 + scaleFactors[1];

        rect.params.factorX = lerp(rect.params.factorX, midiFactorX * Math.abs(row - frequencyFactor), 0.01);
        rect.params.factorY = lerp(rect.params.factorY, midiFactorY * Math.abs(col - frequencyFactor / rows * cols), 0.01);
        rect.params.offsetIndex = lerp(rect.params.offsetIndex, midiFactorY * midiFactorX, 0.005);
  ...
  [Scenes.SOLO_03]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        speedFactor1: woodwinds.centroid || knob1,
        speedFactor2: bass.pitch || knob3,
        scaleFactor1: woodwinds.loudness || knob2,
        scaleFactor2: bass.loudness || knob4,
        frequencyFactor: woodwinds.loudness || knob5,
      }

      // Constants
      const BASE_FREQ = time * 0.001;
      const SCALE_RANGE = { min: 0.1, max: 20 };
      ...
      const waveFrequency = BASE_FREQ * 0.01 + _state.frequencyProgress;
      const motionFrequency = BASE_FREQ * 0.01 + _state.frequencyProgress;
      const frequencyFactor = _input.frequencyFactor;
      ...
      elements.grid?.data.forEach((rect, i) => {
        ...
        const scaleFactor = (0.35 - Math.abs(row - rows * scaleFactors[0]) * 0.11) + (0.35 - Math.abs(row - rows * scaleFactors[1]) * 0.09);
        const midiFactorY = Math.sin(scaleFactors[0] + row * 0.5) * Math.floor(Math.sin(Math.PI * 0.15 * row * frequencyFactor) * 3) + scaleFactors[0];
        const midiFactorX = 0.01 + scaleFactors[1];
        ...
        const mainWave = Math.sin(waveFrequency * colSpeedMult + rect.params.factorY);
        const subWave = Math.cos(waveFrequency * 0.5 + row * Math.PI * 0.15 + rect.params.factorX);
        const combined = (mainWave * 0.55 + subWave * 0.45) * scaleFactor;
        const scaleValue = Easing.CIRC_IN(Math.abs(combined));
  ...
  [Scenes.SOLO_04]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        speedFactor: midikeys.express_and_rotary[2] || knob1,
        speedFactor2: knob3, // to implement
        scaleFactor: keys.loudness || knob2,
        scaleFactor2: knob4, // to implement
        frequencyFactor: keys.loudness || knob5,
      }
      ...
      elements.grid?.data.forEach((rect, i) => {
        ...
        rect.position.x += Math.sin(motionFrequency * rect.params.factorY + i * rect.params.offsetIndex) * rect.params.rowFactor * 15;
  ...
  [Scenes.STAYS_NOWHERE]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        distanceFactor1: woodwinds.loudness,
        ...
        scaleFactor: knob1 || keys.loudness,
      ...
      const SCALE_RANGE = { min: 0.5, max: 2.5 }
      const SCALE_DISTANCE_RANGE = { min: 100, max: 600 };
      const CONNECTION_RANGE = { min: 150, max: 750 };
      const CONNECTION_CHANCE = 0.033;

      // Computed audio values + MIDI
      const scaleFactor = mapLinear(_input.scaleFactor, 0, 1, SCALE_RANGE.min, SCALE_RANGE.max);
  ...
  [Scenes.STRANGE_ATTRACTOR]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        orbitFactor1: knob1 || brass.loudness,
        orbitFactor2: knob2 || woodwinds.loudness,
        orbitFactor3: knob3 || bass.loudness,
        orbitFactor4: keys.loudness,
        cameraRotationFactor: knob4 || harmonies.loudness,
        cameraZoomSpeed: knob5 || bassDrum.loudness,
        cameraZoomFactor: knob6 || drums.loudness,
      }
      ...
      elements.rings.forEach(ring => {
        ...
        ring?.data.forEach((rect, i) => {
          ...
          if (ended) {
            rect.position.x = lerp(rect.position.x, 0, 0.0033);
            rect.position.y = lerp(rect.position.y, 0, 0.0041);
            rect.position.z = lerp(rect.position.z, 0, 0.0027);
          }
          else {
  ...
  [Scenes.TUFTEEE]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        scaleFactor3: keys.centroid || knob3,
        globalSpeedFactor: brass.centroid || knob4,
      ...
      // Computed audio values + MIDI
      const globalScaleFrequency = BASE_FREQ * 2 * _input.globalSpeedFactor;
      ...
      const scrollDirection = _input.scrollDirection > 0 ? Math.sign(_input.scrollDirection - 0.5) : random([-1, 1]);
      const rowHeight = elements.grid.config.style.size.y;
      const rows = elements.grid.config.layout.dimensions?.y || 10;

      _state.globalSpeed = lerp(_state.globalSpeed, globalScaleFrequency, 0.01)
      ...
      elements.grid.data.forEach((rect, i) => {
        ...
        // const speedFactor = speedFactors[i % speedFactors.length] * 4;
        ...
        if (rect.motionSpeed) {
          if (!ended) {
            rect.renderScale.x = 1 * Easing.POWER2_IN_OUT(Math.sin(_state.globalSpeed + i * 0.08 * (0.05 + scaleFactor))) * scaleFactor;
          }
          else {
      ...
      repeatEvery({ beats: 3 }, () => {
        ...
        elements.grid?.data.forEach((rect, i) => {
          ...
          // Reset black color
            ...
            const speedFactor = 0.001 * (0.15 + speedFactors[row % speedFactors.length]);
            const distanceFromTarget = Math.abs(randomIndex - row) * offsetFactor;
            const direction = scrollModeX == 1 ? 1 : randomIndex % 2 === 0 ? 1 : -1;
  ...
  [Scenes.USBTEC]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        cameraZoomFactor: bassDrum.loudness || overhead.loudness,
        cameraZoomFactor2: liveFx.loudness,
        cameraAngleFactor: bass.loudness,
      }
      ...
      const { azimuth, polar } = engine.getCameraAngles();
      const cameraPos = engine.getCameraPosition();
      const cameraZoom = (_camera.speedZoom || 0) * beatCycle(time, { beats: 2, offset: 2 }) * (_input.cameraZoomFactor + _input.cameraZoomFactor2);
  ...
  [Scenes.ZENO]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        speedFactorBack: overhead.loudness || knob1,
        rotationFactorFront: brass.pitch,
        rotationFactorBack: woodwinds.pitch,
        pushFactor1: woodwinds.loudness || knob2,
        pushFactor2: brass.loudness,
        changeSpeedChance: harmonies.centroid || knob3,
        changeRowChance: bassDrum.loudness || knob4,
        changeRowTrigger: bassDrum.onOff,
        cameraRotationFactor: brass.loudness || knob5,
      ...
      _state.pushFactor = lerp(_state.pushFactor, _input.pushFactor1 + _input.pushFactor2, 0.05);
      const pushFactor = _state.pushFactor;

      // --- 2. GLOBAL & CAMERA SECTION ---
      const { azimuth, polar } = engine.getCameraAngles();
      const cameraAngleX = azimuth + Easing.SINE_IN(_input.cameraRotationFactor) * (_camera.speedAngleX || 0);
      const cameraZoom = (_camera.speedZoom || 0) * beatCycle(time, { beats: 12 }) * _input.cameraZoomFactor;
  ...
  [Scenes.ZOHO]: {
    ...
    update: (engine, time) => {
      ...
      _input = {
        ...
        scansTrigger: pad1,
        cameraRotationFactor: woodwinds.loudness,
      }
      ...
      const { azimuth, polar } = engine.getCameraAngles();
      const cameraPos = engine.getCameraPosition();
      const cameraAngleX = azimuth + (_camera.speedAngleX || 0) * (1 + _input.cameraRotationFactor);
